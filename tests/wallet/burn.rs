...
fn inscriptions_can_be_burned() {
  ...
  let (inscription, _) = inscribe_with_options(&core, &ord, Some(10000), None, 1);
...
fn runic_outputs_are_protected() {
  ...
  let (inscription, _) = inscribe_with_options(&core, &ord, Some(1000), None, 1);
...
fn cannot_burn_inscriptions_on_large_utxos() {
  ...
  let (inscription, _) = inscribe_with_options(&core, &ord, Some(10_001), None, 1);
...
fn cannot_burn_inscription_sharing_utxo_with_another_inscription() {
  ...
  let (inscription0, _) = inscribe_with_options(&core, &ord, Some(1000), None, 0);
  let height0 = core.height();
  let (inscription1, _) = inscribe_with_options(&core, &ord, Some(1000), None, 0);
  let height1 = core.height();
  let (inscription2, _) = inscribe_with_options(&core, &ord, Some(1000), None, 0);

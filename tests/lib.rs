...
fn inscribe_with_options(
  ...
  let mut command_str = format!(
    "--chain {} --index-relics wallet inscribe --fee-rate {} --file foo.txt",
    core.network(),
    fee_rate,
  );
  ...
  if let Some(parent) = parent {
    command_str.push_str(&format!(" --parent {}", parent));
  }
...
fn inscribe_with_parent(
  ...
  let (inscription, reveal) = inscribe_with_options(core, ord, None, Some(parent), 1);
  (inscription, reveal)
}

fn inscribe(core: &mockcore::Handle, ord: &TestServer) -> (InscriptionId, Txid) {
  inscribe_with_options(core, ord, None, None, 1)
}
...
fn batch(core: &mockcore::Handle, ord: &TestServer, batchfile: batch::File) -> Etched {
  ...
  let mut builder = CommandBuilder::new(
    "--regtest --index-runes --index-relics wallet batch --fee-rate 0 --batch batch.yaml",
  )
...
fn relic_balance(
  ...
  let command = "--chain regtest --index-relics wallet balance";
...
fn relic_mint(
  ...
  // if slippage, test caller manually needs to mine
  if slippage.is_none() {
    core.mine_blocks(1);
  }

  let command_name = if relic == SpacedRelic::from_str("MBTC").unwrap() {
    "mint-base-relic"
  } else {
    "mint-relic"
  };

  let mut command = if command_name == "mint-base-relic" {
    format!(
      "--chain regtest --index-relics wallet {} --fee-rate 1",
      command_name
    )
  } else {
    format!(
      "--chain regtest --index-relics wallet {} --fee-rate 1 --relic {}",
      command_name, relic
    )
  };

  if num_mints > 0 {
    command.push_str(&format!(" --num-mints {}", num_mints));
  }

  if let Some(slippage) = slippage {
    command.push_str(&format!(" --slippage {}", slippage));
  }

  if let Some(inscription) = inscription {
    command.push_str(&format!(" --inscription {}", inscription));
  }
  ...
  pretty_assert_eq!(output.relic, Some(relic));
...
fn relic_unmint(core: &mockcore::Handle, ord: &TestServer, relic: SpacedRelic) {
  core.mine_blocks(1);

  let command = format!(
    "--chain regtest --index-relics wallet mint-relic --unmint --fee-rate 1 --relic {}",
    relic,
  );
...
fn relic_enshrine(
  ...
  let output = CommandBuilder::new(command)
    .core(core)
    .ord(ord)
    .run_and_deserialize_output::<ord::subcommand::wallet::enshrine::Output>();

  pretty_assert_eq!(output.relic, relic);

  let tx = u32::try_from(core.mine_blocks(1).first().unwrap().txdata.len() - 1).unwrap();
  let block = core.height();

  RelicId { block, tx }
}

fn relic_enshrine_free(core: &mockcore::Handle, ord: &TestServer, relic: SpacedRelic) -> RelicId {
  let command = format!(
    r#"
        --chain regtest
        --index-relics
...
fn relic_batch(core: &mockcore::Handle, ord: &TestServer, batchfile: batch::File) -> Batch {
  let mut builder =
    CommandBuilder::new("--regtest --index-relics wallet batch --fee-rate 0 --batch batch.yaml")
      .write("batch.yaml", serde_yaml::to_string(&batchfile).unwrap())
      .core(core)
      .ord(ord);
  ...
  spawn.run_and_deserialize_output::<Batch>()
}

fn seal(core: &mockcore::Handle, ord: &TestServer, relic: SpacedRelic) -> InscriptionId {
  core.mine_blocks(1);

  let batch = relic_batch(
    core,
    ord,
    batch::File {
      sealing: true,
      inscriptions: vec![batch::Entry {
        file: Some("inscription.jpeg".into()),
        // put the Relic Ticker into the metadata
        metadata: Some(relic.to_metadata_yaml()),
        ..default()
      }],
      ..default()
    },
  ...
  let output = CommandBuilder::new("--chain regtest --index-relics wallet sealed")
    .core(core)
    .ord(ord)
    .run_and_deserialize_output::<Vec<ord::subcommand::wallet::sealed::Output>>();

  pretty_assert_eq!(output.last().unwrap().relic, relic);

  core.mine_blocks(1);
  ord.sync_server();

  batch.inscriptions.first().unwrap().id
}

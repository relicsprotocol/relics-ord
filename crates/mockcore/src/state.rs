...
impl State {
  ...
  pub(crate) fn broadcast_tx(&mut self, template: TransactionTemplate) -> Txid {
    ...
    for outpoint in template.input_outpoints.iter() {
      total_value += self
        .utxos
        .get(outpoint)
        .map(|amount| amount.to_sat())
        .or_else(|| {
          // if the Outpoint is not found check the mempool
          self
            .mempool
            .iter()
    ...
    let value_per_output = if template.outputs > 0 {
      (total_value - template.fee - template.op_return_value.unwrap_or_default())
        / template.outputs as u64
    } else {
      0
    };

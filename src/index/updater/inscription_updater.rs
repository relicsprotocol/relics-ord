use {super::*, crate::relics::INCEPTION_PARENT_INSCRIPTION_ID, std::str::FromStr};
...
pub(super) struct InscriptionUpdater<'a, 'tx, 'emitter> {
  ...
  pub(super) store_only_relics: bool,
...
impl InscriptionUpdater<'_, '_, '_> {
  pub(super) fn index_inscriptions(
    ...
    let mut has_new_inscriptions = false;
    ...
    for (input_index, txin) in tx.input.iter().enumerate() {
      ...
      while let Some(inscription) = envelopes.peek() {
        ...
        let keep = if !self.store_only_relics {
          true
        } else {
          let inception_parent_inscription_id =
            InscriptionId::from_str(INCEPTION_PARENT_INSCRIPTION_ID).unwrap();

          // Check if the inscription itself is the inception parent
          let is_inception_parent = inscription_id == inception_parent_inscription_id;

          // otherwise require relic parent or relic metadata
          let first_parent_ok = inscription
            .payload
            .parents()
            .first()
            .is_some_and(|p| *p == inception_parent_inscription_id);

          let relic_in_meta = inscription
            .payload
            .metadata()
            .is_some_and(|m| SpacedRelic::from_metadata(m).is_some());

          is_inception_parent || first_parent_ok || relic_in_meta
        };

        if keep {
          has_new_inscriptions = true;
        }
    ...
    if cfg!(test) {
      potential_parents = floating_inscriptions
        .iter()
        .map(|flotsam| flotsam.inscription_id)
        .collect::<HashSet<InscriptionId>>();
      potential_parents.insert(InscriptionId::from_str(INCEPTION_PARENT_INSCRIPTION_ID)?);
    } else {
      potential_parents = floating_inscriptions
        .iter()
        .map(|flotsam| flotsam.inscription_id)
  ...
  fn update_inscription_location(
    ...
    let (unbound, sequence_number) = match flotsam.origin {
      Origin::Old {
        ...
        if store {
          let current_inscription_entry_value = self
            .sequence_number_to_entry
            .get(&sequence_number)?
            .unwrap()
            .value();
          let mut current_inscription_entry =
            InscriptionEntry::load(current_inscription_entry_value);

          // Determine if it's a base relic BEFORE potential move by .store()
          let inception_parent_inscription_id =
            InscriptionId::from_str(INCEPTION_PARENT_INSCRIPTION_ID).unwrap();
          let inception_parent_sequence_number_option = self
            .id_to_sequence_number
            .get(&inception_parent_inscription_id.store())?
            .map(|id_entry_val| id_entry_val.value());

          let is_base_relic = match inception_parent_sequence_number_option {
            Some(parent_seq_num) => current_inscription_entry.parents.contains(&parent_seq_num),
            None => false,
          };

          // If op_return is true, modify charms and update the stored entry
          if op_return {
            Charm::Burned.set(&mut current_inscription_entry.charms);
            // .store() moves current_inscription_entry. This is now fine as is_base_relic is determined.
            self
              .sequence_number_to_entry
              .insert(sequence_number, &current_inscription_entry.store())?;
          }
          ...
          self.event_emitter.emit(
            ...
            EventInfo::InscriptionTransferred {
              ...
              sequence_number,
              is_base_relic, // Use the pre-calculated value
            },
          )?;
        }

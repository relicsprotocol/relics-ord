...
impl Plan {
  pub(crate) fn inscribe(
    ...
    relic_balances: BTreeMap<OutPoint, BTreeMap<SpacedRelic, Pile>>,
  ...
  pub(crate) fn create_batch_transactions(
      ...
      utxos
        ...
        .find(|(outpoint, txout)| {
          txout.value.to_sat() > 0
            ...
            && !relic_balances.contains_key(outpoint)
        })
    ...
    let mut required_relics: BTreeMap<SpacedRelic, u128> = BTreeMap::new();
    let base_token = SpacedRelic::from_str(RELIC_NAME).unwrap();

    if self.sealing {
      // only one Relic ticker can be sealed per transaction, and it has to be sealed to the first inscription
      let metadata = self
        .inscriptions
        .first()
        .expect("sealing requires an inscription")
        .metadata()
        .expect("sealing inscription must contain metadata");
      let sealing_fee = SpacedRelic::from_metadata(metadata)
        .expect("sealing inscription metadata must contain a valid Relic ticker")
        .relic
        .sealing_fee();
      // add RELIC to cover the sealing fee
      *required_relics.entry(base_token).or_default() += sealing_fee;
    }

    let (mut inputs, input_relic_balances) =
      Wallet::select_required_relic_outputs(&relic_balances, &required_relics)?;

    // add all relic inputs to the reveal TX
    reveal_inputs.append(&mut inputs);

    // add a change output if relics were overpaid
    let relic_change_output = if input_relic_balances
      .iter()
      .any(|(relic, amount)| *amount > required_relics.get(relic).cloned().unwrap_or_default())
    {
      let output = u32::try_from(reveal_outputs.len()).unwrap();
      reveal_outputs.push(TxOut {
    ...
    let keepsake = Keepsake {
      ...
      pointer: relic_change_output,
      ..default()
    };

    // drop a protocol message that equals the default as it would be empty anyway
    let keepsake = (keepsake != Keepsake::default()).then_some(keepsake);

    if let Some(keepsake) = &keepsake {
      reveal_outputs.push(TxOut {
    ...
    if relic_change_output.is_some() {
      target_value += MIN_POSTAGE;
    }
    ...
    let unsigned_commit_tx = TransactionBuilder::new(
      ...
      utxos.clone(),
      locked_utxos.clone(),
      relic_balances.keys().cloned().collect(),
      runic_utxos,
    ...
    for input in reveal_inputs.iter().skip(prevouts.len()) {
      prevouts.push(utxos.get(input).unwrap().clone());
    }

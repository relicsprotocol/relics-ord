use {
  ...
  crate::templates::{
    ...
    PreviewVideoHtml, RareTxt, RelicEventsHtml, RelicHtml, RelicsHtml, RuneHtml, RunesHtml,
    SatHtml, SealingHtml, SealingsHtml, TransactionHtml,
  },
  axum::{
    body,
...
impl Server {
  pub fn run(self, settings: Settings, index: Arc<Index>, handle: Handle) -> SubcommandResult {
    Runtime::new()?.block_on(async {
      ...
      let server_config = Arc::new(ServerConfig {
        ...
        relics_light_mode: settings.store_relics_only(),
      });
      ...
      let router = Router::new()
        ...
        .route("/events/:relic/:page", get(Self::relic_events_paginated))
        .route("/events/:block", get(Self::block_events))
        .route("/sealing/:relic", get(Self::sealing_info))
        .route("/sealings/:page", get(Self::sealings_paginated))
        .route("/relic/:relic", get(Self::relic))
        .route("/relics", get(Self::relics))
        .route("/relics/:page", get(Self::relics_paginated))
        .route("/relics/balances", get(Self::relics_balances))
        .route("/relics/claimable", get(Self::relics_claimable))
        .route(
          "/relics/decipher/:txid",
          get(Self::decipher_transaction_relics),
        )
        .route(
          "/relics/decipher/raw/:hex_tx",
          get(Self::decipher_raw_transaction_relics),
        )
  ...
  async fn outputs(
    ...
    task::block_in_place(|| {
      Ok(if accept_json {
        let mut response: Vec<Option<api::Output>> = Vec::new();
        for outpoint in outputs {
          let maybe_output_tuple = index.get_output_info(outpoint)?;

          if let Some((output_data, _tx_out)) = maybe_output_tuple {
            response.push(Some(output_data));
          } else {
            response.push(None);
          }
        }
    ...
    task::block_in_place(|| {
      ...
      let relic = match relic_query {
        ...
        query::Relic::Id(relic_id) => index
          ...
          .ok_or_not_found(|| format!("relic {relic_id}"))?,
        query::Relic::Number(number) => index
          .get_relic_by_number(usize::try_from(number).unwrap())?
          .ok_or_not_found(|| format!("relic number {number}"))?,
      };
      ...
      if entry.enshrining != Txid::all_zeros() {
        ...
        for (vout, _output) in transaction.output.iter().enumerate() {
          let outpoint = OutPoint::new(
            enshrining_txid,
            u32::try_from(vout).map_err(|e| ServerError::Internal(anyhow::anyhow!(e)))?,
          );
      ...
      let mintable = entry.mintable(u128::MAX, 1, u128::MAX).is_ok();
  ...
  async fn sealing_info(
    ...
    task::block_in_place(|| {
      ...
      let inscription = if let Some(inscription) = entry.0 {
        ...
        return Err(ServerError::BadRequest(format!("relic {relic} not found")));
      };
  ...
  async fn block(
    ...
    task::block_in_place(|| {
      ...
      let relics = index.get_relics_in_block(u64::from(height))?;
  ...
  async fn transaction(
    ...
    task::block_in_place(|| {
      ...
      let events = index.events_for_tx(txid)?;
  ...
  async fn decipher_transaction_relics(
    Extension(index): Extension<Arc<Index>>,
    Path(txid): Path<Txid>,
    AcceptJson(accept_json): AcceptJson,
  ) -> ServerResult {
    task::block_in_place(|| {
      ...
      let relic_artifact = Keepsake::decipher(&transaction);

      Ok(if accept_json {
        Json(relic_artifact).into_response()
      } else {
  ...
  async fn decipher_raw_transaction_relics(
    Path(hex_tx): Path<String>,
    AcceptJson(accept_json): AcceptJson,
  ) -> ServerResult {
    task::block_in_place(|| {
      let tx_bytes = hex::decode(&hex_tx)
        .map_err(|e| ServerError::BadRequest(format!("Invalid hex transaction: {}", e)))?;

      let transaction: bitcoin::Transaction = consensus::deserialize(&tx_bytes).map_err(|e| {
        ServerError::BadRequest(format!("Failed to deserialize transaction: {}", e))
      })?;
...
.*",
  ...
  fn runes_are_displayed_on_output_page() {
    ...
    assert_eq!(
      server.index.get_rune_balances().unwrap(),
      [(outpoint, vec![(id, u128::MAX)])]
    );

    server.assert_response_regex(
      format!("/output/{outpoint}"),
      StatusCode::OK,
      format!(
        ".*<title>Output {outpoint}</title>.*<h1>Output <span class=monospace>{outpoint}</span></h1>.*
  <dt>runes</dt>
  <dd>
    <table>
      <tr>
...
.*"
    ...
    pretty_assert_eq!(
      server.get_json::<api::Output>(format!("/output/{outpoint}")),
      api::Output {
        outpoint,
        value: 5000000000,
  ...
  fn nav_displays_chain() {
    TestServer::builder()
      ...
      .assert_response_regex(
        "/",
        StatusCode::OK,
        ".*<a href=/ title=home>Relics<sup>regtest</sup></a>.*",
      );
  }
...
",
  ...
  fn dont_show_rare_txt_in_header_without_sat_index() {
    TestServer::new().assert_response_regex(
      ...
      ".*
      <a href=/clock title=clock>.*</a>
      <a href=https://docs.relicsprotocol.com.*",
    );
  }

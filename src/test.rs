...
pub(crate) fn inscription(content_type: &str, body: impl AsRef<[u8]>) -> Inscription {
  inscription_with_meta(content_type, body, None, None)
}

pub(crate) fn inscription_with_meta(
  content_type: &str,
  body: impl AsRef<[u8]>,
  metaprotocol: Option<&str>,
  metadata_map: Option<BTreeMap<String, Value>>,
) -> Inscription {
  let metadata = metadata_map.map(|map| {
    let mut buffer = Vec::new();
    ciborium::into_writer(&map, &mut buffer).expect("Serialization failed");
    buffer
  });
  ...
  Inscription {
    ...
    metaprotocol: metaprotocol.map(|s| s.as_bytes().to_vec()),
    metadata,
    ..default()
  }
}

use {
  self::{
    entry::{
      ...
      OutPointValue, RelicEntryValue, RelicIdValue, RelicOwnerValue, RuneEntryValue, RuneIdValue,
...
pub use self::{
  entry::SpacedRelicValue,
  event::{Event, EventInfo, RelicOperation},
};
...
impl Index {
  ...
  pub fn open_with_event_sender(
      ...
      .open(&path)
    {
        ...
        if error.kind() == io::ErrorKind::NotFound =>
      {
...

        {
          ...
          Self::set_statistic(
            &mut statistics,
            Statistic::IndexRelics,
            u64::from(settings.index_relics_raw()),
          )?;
        ...
        if settings.index_relics_raw() {
          ...
          Self::set_statistic(&mut statistics, Statistic::Relics, 1)?;
          ...
          tx.open_table(RELIC_ID_TO_RELIC_ENTRY)?.insert(
            ...
            RelicEntry {
              ...
              mint_terms: Some(MintTerms {
                // mint amount per burned relic inscription = ~21M total supply
                amount: Some(654_205_000_000),
                block_cap: None,
                // total amount of relic child inscriptions
                cap: Some(3210),
                max_unmints: None,
                price: None,
                seed: None,
...

    {
      ...
      index_relics = Self::is_statistic_set(&statistics, Statistic::IndexRelics)?;
  ...
  pub fn status(&self, json_api: bool) -> Result<StatusHtml> {
    ...
    Ok(StatusHtml {
      ...
      relic_index: self.has_relic_index(),
      relics: statistic(Statistic::Relics)?,
  ...
  pub fn relics_paginated(
    ...
    page_size: usize,
    page_index: usize,
  ) -> Result<(Vec<(RelicId, Option<InscriptionId>, RelicEntry)>, bool)> {
    let mut entries = Vec::new();
    let rtx = self.database.begin_read()?;
    ...
    {
      let (id, entry) = result?;
      let relic_id = RelicId::load(id.value());
      let relic_entry = RelicEntry::load(entry.value());

      // Find sealing inscription
      let sealing_inscription = if let Some(seq_number) = relic_to_sequence_number
        .get(&relic_entry.spaced_relic.relic.store())?
        .map(|v| v.value())
      {
        sequence_number_to_inscription_entry
          .get(seq_number)?
          .map(|insc| InscriptionEntry::load(insc.value()).id)
      } else {
        None
      };

      entries.push((relic_id, sealing_inscription, relic_entry));
    }
  ...
  pub fn sealing(&self, relic: Relic) -> Result<(Option<api::Inscription>, Option<Txid>)> {
    ...
    if let Some(seq_number) = relic_to_sequence_number.get(&relic.store())? {
      ...
      if let Some(inscription_entry_val) = sequence_number_to_inscription_entry.get(seq_number)? {
        ...
        if let Some((api_inscription, _, _)) =
          self.inscription_info(query::Inscription::Id(inscription_id), None)?
        {
  ...
  pub fn get_enshrining(&self, txid: Txid) -> Result<Option<SpacedRelic>> {
    let rtx = self.database.begin_read()?;

    let transaction_id_to_relic = rtx.open_table(TRANSACTION_ID_TO_RELIC)?;
    let Some(relic) = transaction_id_to_relic.get(&txid.store())? else {
      return Ok(None);
    };

    let relic_to_relic_id = rtx.open_table(RELIC_TO_RELIC_ID)?;
    let id = relic_to_relic_id.get(relic.value())?.unwrap();

    let relic_id_to_relic_entry = rtx.open_table(RELIC_ID_TO_RELIC_ENTRY)?;
    let entry = relic_id_to_relic_entry.get(&id.value())?.unwrap();

    Ok(Some(RelicEntry::load(entry.value()).spaced_relic))
  }
  ...
  pub fn get_relics_in_block(&self, block_height: u64) -> Result<Vec<SpacedRelic>> {
    let rtx = self.database.begin_read()?;
    ...
    let relics = relic_id_to_relic_entry
      .range(min_id.store()..=max_id.store())?
      .map(|result| result.map(|(_, entry)| RelicEntry::load(entry.value()).spaced_relic))
      .collect::<Result<Vec<SpacedRelic>, StorageError>>()?;
  ...
  pub(crate) fn get_output_info(&self, outpoint: OutPoint) -> Result<Option<(api::Output, TxOut)>> {
    ...
    let inscription_ids = self.get_inscriptions_for_output(outpoint)?;

    // Convert inscription IDs to RelicInscription structs
    let mut relic_inscriptions = Vec::with_capacity(inscription_ids.len());
    ...
    for id in &inscription_ids {
      let is_base_relic = if let Ok(Some(sequence_number)) =
        inscription_id_to_sequence_number.get(&id.store())
      {
        let entry = InscriptionEntry::load(
          sequence_number_to_inscription_entry
            .get(sequence_number.value())?
        ...
        // Check each parent to see if it matches INCEPTION_PARENT_INSCRIPTION_ID
        let mut found_base_parent = false;
        for parent_sequence in &entry.parents {
          if let Ok(Some(parent_entry)) = sequence_number_to_inscription_entry.get(parent_sequence)
          {
            let parent_id = InscriptionEntry::load(parent_entry.value()).id;
            if parent_id.to_string() == INCEPTION_PARENT_INSCRIPTION_ID {
              found_base_parent = true;
              break;
            }
          }
        }
      ...
      relic_inscriptions.push(api::RelicInscription {
        id: *id,
        is_base_relic,
      });
    }

    let relics = self.get_relic_balances_for_outpoint(outpoint)?;
    ...
    Ok(Some((
      api::Output {
        ...
        indexed,
        inscriptions: relic_inscriptions,
        outpoint,
        relics,
        runes,
        sat_ranges,
  ...
  fn inscription_event_sender_channel() {
    ...
    assert_eq!(
      ...
      Event {
        ...
        event_index: 0,
        txid: transfer_txid,
        info: EventInfo::InscriptionTransferred {
          is_base_relic: false,
          inscription_id,
  ...
  fn rune_event_sender_channel() {
    ...
    assert_eq!(
      ...
      Event {
        ...
        event_index: 0,
        txid: txid0,
        info: EventInfo::RuneEtched { rune_id: id },
      }
    );
    ...
    pretty_assert_eq!(
      ...
      Event {
        ...
        event_index: 0,
        txid: txid2,
        info: EventInfo::RuneTransferred {
          rune_id: id,
          amount: 1000,
          output: 0,
        },
